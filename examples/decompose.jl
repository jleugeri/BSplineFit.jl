...
plts = []
for order ∈ 1:3
    p = scatter(data_x, data_y, label="", title="Decompose data using BSpline basis of order $(order).")
    for id ∈ 1:length(knots)+order
        b = BSplineFit.BSpline1DBasis(order, knots, id)
        plot!(t->b(t), 0, 7, lw=2, ls=:dash, label="")
    end
    
    pos,cmp = decompose(data_x, data_y, order, knots)
    plot!(pos, cmp, marker=:o, lc=:black, lw=2, label="", ylims=(-0.5,1.5))
...
savefig(p, "examples/decompose.svg")
